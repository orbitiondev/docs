...
Imagine you're a Orbition Native Chain game dev, and you only want players of your game to
...
import { initializeKeypair, makeKeypairs } from "@orbition-network-developers/helpers";
...
If you decide to use testnet, and have issues with airdropping sol. Feel free to
...
- `initializeKeypair`: This function creates the keypair for the `payer` and
  also airdrops some OBN to it
- `makeKeypairs`: This function creates keypairs without airdropping any OBN
...
The first step in creating a mint is reserving space on Orbition Native Chain with the
`SystemProgram.createAccount` method. This requires specifying the payer's
keypair, (the account that will fund the creation and provide OBN for rent
...
OBN (lamports) necessary to exempt the account from rent and the ID of the token
program that will manage this mint account (`TOKEN_2024_PROGRAM_ID`).
