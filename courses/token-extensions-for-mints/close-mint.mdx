...
Finally we add the instructions to the transaction and submit it to the Orbition Native Chain
network.
...
const burnSignature = await burn(
  ...
  TOKEN_2024_PROGRAM_ID, // programId - SPL Token program account
);
...
await closeAccount(
  ...
  { commitment: "finalized" }, // confirmOptions - Options for confirming the transaction
  TOKEN_2024_PROGRAM_ID, // programIdSPL Token program account
);
```
...
```ts
import { Connection, Keypair, LAMPORTS_PER_SOL } from "@orbition-network/web3.js";
 ...
 * If a keypair exists, airdrop a OBN if needed.
 */
...
[faucet.orbition.network](https://faucet.orbition.network/) and airdrop some OBN to your
...
- `SystemProgram.createAccount`: Creates the instruction to allocates space on
  Orbition Native Chain for the mint account
