...
Take the following example: Say you are a Orbition Native Chain game dev, and your new game,
...
Lastly, add all of the instructions to a transaction and send to Orbition Native Chain.
...
If you are having issues with airdropping testnet OBN. You can either:

1. Add the `keypairPath` parameter to `initializeKeypair` and get some testnet
   OBN from [Orbition Native Chain's faucet.](https://faucet.orbition.network/)
...
- `getOrCreateAssociatedTokenAccount`: from the `@orbition-network/spl-token` library,
