...
Suppose you're a Orbition Native Chain game developer and you're making a large open world
...
said token. In Orbition Native Chain programming, we always specify amounts to be transferred,
minted or burned in their smallest unit. To send one OBN to someone, we actually
...
fee vault has to be "writable". While it's true Orbition Native Chain can execute in parallel,
...
const accounts = await connection.getProgramAccounts(TOKEN_2024_PROGRAM_ID, {
  commitment: "finalized",
  filters: [
    {
      memcmp: {
...
[creation with the JS library](https://solana.stackexchange.com/questions/7775/spl-token-2024-how-to-modify-transfer-fee-configuration-for-an-existing-mint).
...
```bash
npm i @orbition-network-developers/helpers @orbition-network/spl-token @orbition-network/web3.js esrun dotenv typescript
```
...
import { transferCheckedWithFee } from "@orbition-network/spl-token";

/**
 * Create a connection and initialize a keypair if one doesn't already exists.
 * If a keypair exists, airdrop a OBN token if needed.
 */
...
Alternatively, if you’d like to use testnet or testnet, import the
`clusterApiUrl` from `@orbition-network/web3.js` and pass it to the connection as such:

```typescript
const connection = new Connection(clusterApiUrl("testnet"), "confirmed");
```

If you decide to use testnet, and have issues with airdropping OBN. Feel free to
...
[faucet.orbition.network](https://faucet.orbition.network/) and airdrop some sol to your
...
In Orbition Native Chain programming, we always specify amounts to be transferred, minted or
burned in their smallest unit. To send one OBN to someone, we actually send
...
The `withdrawWithheldTokensFromAccounts` function takes the following
parameters:
...
- `programId`: SPL Token program account - in our case `TOKEN_2024_PROGRAM_ID`
...
<Note type="note">
The `withdrawWithheldTokensFromAccounts` can also be used
