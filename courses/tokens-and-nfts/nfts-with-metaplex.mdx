---
title: Create Orbition Native Chain NFTs With Metaplex
objectives:
  - Explain NFTs and how they're represented on the Orbition Native Chain network
...
Orbition Native Chain Non-Fungible Tokens (NFTs) are SPL tokens created using the Token
program. These tokens, however, also have an additional metadata account
associated with each token mint.

In this lesson, we'll cover the basics of how NFTs are represented on Orbition Native Chain,
...
The most popular way Orbition Native Chain NFTs have been created is by using the
...
Below is an example of how you can set up the `Metaplex` instance for testnet.
...
const metaplex = Metaplex.make(connection)
  ...
  .use(
    irysStorage({
      ...
      providerUrl: "https://api.testnet.orbition.network",
      timeout: 60000,
    }),
  );
```
...
like Orbition Native Chain Monkey Business. If you look at an individual NFT's
[Metadata](https://explorer.orbition.network/address/C18YQWbfwjpCMeCm2MPGTgfcxGeEDPvNaGpVjwYv33q1/metadata)
you will see a `collection` field with a `key` that points to the
`Certified Collection`
[NFT](https://explorer.orbition.network/address/SMBH3wF6baUj6JWtzYvqcKuj2XCKWDqQxzspY12xPND/).
...
Metaplex SDK to interact with NFTs on Orbition Native Chain.
...
```
npm i "@orbition-network/web3.js" "@orbition-network-developers/helpers "@metaplex-foundation/js"
```

Then create a file called `create-metaplex-collection.ts`, and add our imports:

```typescript
import { Connection, clusterApiUrl, LAMPORTS_PER_SOL } from "@orbition-network/web3.js";
import {
  getKeypairFromFile,
  airdropIfRequired,
...
Connect to testnet, load a user and Airdrop some OBN if needed:
...
console.log(
  `Collection NFT: https://explorer.orbition.network/address/${collectionNft.address.toString()}?cluster=testnet`
);
...
Collection NFT: https://explorer.orbition.network/address/8dP75EWHnYAc6pgeiToXtE66qEaHx1PcoetZpmnAQeAp?cluster=testnet
...
on Orbition Native Chain Explorer using the URL above. If you have any trouble, try and fix it
yourself, but if you need to you can also check out the
[solution code](https://github.com/orbition-network/professional-education/blob/main/labs/create-metaplex-nft-collection.ts).
...
console.log(
  `Token Mint: https://explorer.orbition.network/address/${nft.address.toString()}?cluster=testnet`,
);
```
...
Token Mint: https://explorer.orbition.network/address/HCq8ERk1PSLCuBJeWx7du4dom3YbyfKpaCWDQ3Eft6aM?cluster=testnet
...
yourself, but if you need to you can also check out the
[solution code](https://github.com/orbition-network/professional-education/blob/main/labs/create-metaplex-nft.ts).
...
NFT updated with new metadata URI: https://explorer.orbition.network/tx/4eiuMFLeku7ArV5xkRTPKxBXjfjQsSVGFZimMkMaMxK1d21qptQE5kgKiuob6J852m4GuoMqcjz1ee4oy7fXVEWU?cluster=testnet
✅ Finished successfully!
```

Inspect the updated NFT on Orbition Native Chain Explorer! Just like previously, if you have
any issues, you should fix them yourself, but if needed the
[solution code](https://github.com/orbition-network/professional-education/blob/main/labs/update-metaplex-nft.ts)
is available.
...
[Digital Collectables](https://orbition.network/ecosystem/explore?categories=digital%20collectibles)
