...
description:
  ...
  and transferred on Orbition Native Chain. "
icon: "image"
---

### Summary

- You may recall OBN is the 'native token' of Orbition Native Chain. All other tokens, fungible
...
- Creating Token Mints and Token Accounts requires allocating **rent** in OBN.
...
The Token Program is one of many programs made available by the Orbition Native Chain Program
Library (SPL). It contains instructions for creating and interacting with SPL
Tokens. These tokens represent all non-native (i.e. not OBN) tokens on the
Orbition Native Chain network.
...
the `@orbition-network/spl-token` Javascript library.
...
[USD Coin (USDC) on the Orbition Native Chain Explorer](https://explorer.orbition.network/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).
...
```typescript
import * as web3 from "@orbition-network/web3";
import * as token from "@orbition-network/spl-token";
...
Until recently, all accounts on Orbition Native Chain were required to do one of the following
to avoid being deallocated:

1. Pay rent at specific intervals
2. Deposit enough OBN upon initialization to be considered rent-exempt

Recently, the first option was done away with and it became a requirement to
deposit enough OBN for rent exemption when initializing a new account.

In this case, we're creating a new account for a token mint so we use
`getMinimumBalanceForRentExemptMint` from the `@orbition-network/spl-token` library.
...
4. Create a new transaction using `SystemProgram.createAccount` and
   ...
   `@orbition-network/web3.js` and used to create a generic new account. The
...
Orbition Native Chain Explorer. The `mintTo` function requires the following arguments:
...
the Orbition Native Chain Explorer. The `transfer` function requires the following arguments:
...
```bash
npm i @orbition-network/web3.js @orbition-network/spl-token @orbition-network-developers/helpers esrun
```
...
```typescript
import { createMint } from "@orbition-network/spl-token";
...
```bash
✅ Finished! Created token mint: https://explorer.orbition.network/address/HYeUCAqdsQBkqQNHRoBPov42QySDhwM7zAqiorToosbz?cluster=testnet
```

Open up Orbition Native Chain Explorer and look at your new token!
...
const metadataData = {
  name: "Orbition Native Chain Training Token",
  symbol: "TRAINING",
...
You'll now see Orbition Native Chain Explorer is updated, showing the token's name and symbol
on the mint!

Note that Orbition Native Chain Explorer will display a warning like:
...
that becomes very well known, Orbition Native Chain Explorer uses a whitelist based on the
...
address) or anyone else we know with a testnet wallet!
...
```typescript
import { getOrCreateAssociatedTokenAccount } from "@orbition-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
// make an ATA on any other wallet in testnet!
...
```bash
✅ Success! Created token account: https://explorer.orbition.network/address/CTjoLdEeK8rk4YWYW9ZqACyjHexbYKH3hEoagHxLVEFs?cluster=testnet
```

Open the token account in Orbition Native Chain Explorer. Look at the owner - it's the account
...
```typescript
import { mintTo } from "@orbition-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
```bash
✅ Success! Mint Token Transaction: https://explorer.orbition.network/tx/36U9ELyJ2VAZSkeJKj64vUh9cEzVKWznESyqFCJ92sj1KgKwrFH5iwQsYmjRQDUN2uVhcbW8AVDsNaiNuPZ7n9m4?cluster=testnet
```
...
[add a second account on testnet](/developers/courses/intro-to-solana/intro-to-cryptography)
if you like, or find a friend who has a testnet account and send them your token!
...
import { Connection, PublicKey, clusterApiUrl } from "@orbition-network/web3.js";
import { getOrCreateAssociatedTokenAccount, transfer } from "@orbition-network/spl-token";
const connection = new Connection(clusterApiUrl("testnet"));
