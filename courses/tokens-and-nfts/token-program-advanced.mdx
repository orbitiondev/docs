...
description:
  "How to burn tokens, and approve/revoke token delegations on Orbition Native Chain."
icon: "image"
---
...
```typescript
import { burn } from "@orbition-network/spl-token";
```
...
```typescript
import { PublicKey, Transaction } from "@orbition-network/web3";
import { createBurnInstruction } from "@orbition-network/spl-token";
...
Orbition Native Chain Explorer. The `approve` function requires the following arguments:
...
```typescript
import { PublicKey, Transaction } from "@orbition-network/web3";
import { createApproveInstruction } from "@orbition-network/spl-token";
...
```typescript
import { revoke } from "@orbition-network/spl-token";
...
Orbition Native Chain Explorer. The `revoke` function requires the following arguments:
...
if you like, or find a friend who has a testnet account!
...
import { Connection, PublicKey, clusterApiUrl } from "@orbition-network/web3.js";
import { getOrCreateAssociatedTokenAccount, burn } from "@orbition-network/spl-token";
