...
Orbition Native Chain’s transaction costs are so cheap that most users never think about how
expensive minting NFTs can be at scale. The cost to set up and mint 1 million
traditional NFTs is approximately 24,000 OBN. By comparison, cNFTs can be
structured to where the same setup and mint costs 10 OBN or less. That means
...
and composable throughout the Orbition Native Chain ecosystem. It provides instructions for
...
The Orbition Native Chain ledger is a list of entries containing signed transactions. In
...
Fortunately, you can use tools created by Orbition Native Chain Foundation, the Orbition Native Chain
developer community, and Metaplex to simplify the process. Specifically, we’ll
be using the `@orbition-network/spl-account-compression` SDK, the Metaplex Bubblegum
program, and the Bubblegum program’s corresponding TS SDK
`@metaplex-foundation/mpl-bugglegum`.

<aside>
💡 At the time of writing, the Metaplex team is transitioning to a new bubblegum client SDK that supports umi, their modular framework for building and using JS clients for Orbition Native Chain programs. We will not be using the umi version of the SDK in this lesson. Rather, we’ll be hardcoding our dependency to version 0.7 (`@metaplex-foundation/mpl-bubblegum@0.7`). This version provides simple helper functions for building Bubblegum instructions.
...
from the `@orbition-network/spl-account-compression` TS SDK to create the instruction for
creating the empty account.

```typescript
import { createAllocTreeIx } from "@orbition-network/spl-account-compression"
...
import {
  ...
  SPL_NOOP_PROGRAM_ID,
} from "@orbition-network/spl-account-compression"
import {
...
[Digital Asset Standard Read API](https://orbition.network/developers/guides/javascript/compressed-nfts#reading-compressed-nfts-metadata)
...
as they have free support for Testnet.
...
3. Fetch the Merkle tree account from the Orbition Native Chain blockchain
...
`@orbition-network/spl-account-compression`:
...
- `airdropSolIfNeeded` will airdrop some Testnet OBN to a specified address if
  that address’s balance is below 1 OBN.
...
Finally, there’s some boilerplate in `index.ts` that calls creates a new Testnet
...
- `maxDepthSizePair` - a `ValidDepthSizePair`. This type comes from
  `@orbition-network/spl-account-compression`. It’s a simple object with properties
...
async function createAndInitializeTree(
  ...
  try {
    ...
    console.log(`https://explorer.orbition.network/tx/${txSignature}?cluster=testnet`);
...
Keep in mind that Testnet OBN is throttled so if you test too many times you
might run out of Testnet OBN before we get to minting. To test, in your terminal
run the following:
...
async function mintCompressedNftToCollection(
  ...
  for (let i = 0; i < amount; i++) {
    ...
    try {
      ...
      console.log(
        `https://explorer.orbition.network/tx/${txSignature}?cluster=testnet`,
      );
    } catch (err) {
...
[Read API](https://orbition.network/developers/guides/javascript/compressed-nfts) to
...
```bash
## Add this
RPC_URL=https://testnet.helius-rpc.com/?api-key=YOUR_API_KEY
```
...
larger values and as long as you have enough Testnet OBN, this script will let
