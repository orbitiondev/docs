...
description: "How to invoke functions in other Orbition Native Chain programs."
icon: "link"
---
...
- CPIs make programs in the Orbition Native Chain ecosystem completely interoperable because
...
and third party programs. CPIs essentially turn the entire Orbition Native Chain ecosystem into
...
(e.g. [@orbition-network/web3.js](https://orbition-network.github.io/solana-web3.js/),
[@orbition-network/spl-token](https://orbition-network.github.io/solana-program-library/token/js/)).
...
There's no need to include a signature because the Orbition Native Chain runtime passes along
...
The Orbition Native Chain runtime will internally
...
[an example transaction that failed with this error](https://explorer.orbition.network/tx/3mxbShkerH9ZV1rMmvDfaAhLhJJqrmMjcsWzanjkARjBQurhf4dounrDCUkGunH1p9M4jEwef9parueyHVw6r2Et?cluster=testnet).
...
[transaction in the explorer](https://explorer.orbition.network/tx/ExB9YQJiSzTZDBqx4itPaa4TpT8VK4Adk7GU5pSoGEzNz9fa7PPZsUxssHGrBbJRnCvhoKgLCWnAycFB7VYDbBg?cluster=testnet).

### Why CPIs matter?

CPIs are a very important feature of the Orbition Native Chain ecosystem and they make all
...
are what makes this possible on Orbition Native Chain.
...
Orbition Native Chain development because they allow programs to control specific addresses in
...
its behalf. When `invoke_signed` is called, the Orbition Native Chain runtime calls
...
The script assumes you're deploying to Testnet. If you're deploying locally, then
...
to test adding reviews and comments. Again, the code assumes you're on Testnet so
