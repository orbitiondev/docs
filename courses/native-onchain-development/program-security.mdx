...
Just as before, we'll be using [Orbition Native Chain Playground](https://beta.solpg.io/) to
...
was just recently enabled by Orbition Native Chain Labs which allows you to dynamically change
...
`Sealed` is Orbition Native Chain's version of Rust's `Sized` trait. This simply specifies that
