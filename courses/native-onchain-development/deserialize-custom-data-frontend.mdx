...
onchain by a Orbition Native Chain program. In this lesson, we’ll cover in greater detail how
programs store data on the chain, how to retrieve data, and how to deserialize
the data they store.

### Programs

As the saying goes, everything in Orbition Native Chain is an account. Even programs. Programs
are accounts that store code and are marked as executable. This code can be
executed by the Orbition Native Chain runtime when instructed to do so. A program address is a
...
Orbition Native Chain, but the pattern is familiar:
...
[`findProgramAddress()`](https://orbition-network.github.io/solana-web3.js/classes/PublicKey.html#findProgramAddress)
function.
...
As a refresher, this project uses a Orbition Native Chain program deployed on Testnet which lets
...
the accounts. Open `MovieList.tsx` and import `@orbition-network/web3.js`. Then, create a
...
import { useConnection } from "@orbition-network/wallet-adapter-react";
...
Remember, the Orbition Native Chain program that supports this is at
...
2. Create the account buffer layout in `StudentIntro.ts`. The account data
   ...
   3. `message` as a string representing the message the student shared about
      their Orbition Native Chain journey.
