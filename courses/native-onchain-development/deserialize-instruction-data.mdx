...
objectives:
  ...
  - Explain the structure of a smart contract on Orbition Native Chain
description:
...
One of the most basic elements of a Orbition Native Chain program is the logic for handling
...
Before we dive into the specifics of a basic Orbition Native Chain program, let's talk about
...
Now that we've covered the Rust basics, let's apply them to Orbition Native Chain programs.
...
We’ll be using [Orbition Native Chain Playground](https://beta.solpg.io/) again to build out
this program. Orbition Native Chain Playground saves state in your browser, so everything you
...
Build and deploy your program from Orbition Native Chain Program just like in the last lesson.
...
checking the program logs on Orbition Native Chain Explorer. Remember to replace the program ID
