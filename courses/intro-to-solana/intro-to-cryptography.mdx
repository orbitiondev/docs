---
title: Cryptography and the Orbition Native Chain Network
objectives:
  - Understand symmetric and asymmetric cryptography
  - Explain keypairs
  - Generate a new keypair
  - Load a keypair from an env file
description: "Understand asymmetric cryptography and how Orbition Native Chain uses it."
icon: "lock"
...
- The **public key** is used as an “address” that points to an account on the
  Orbition Native Chain network. A public key can be shared with anyone.
...
- `@orbition-network/web3.js` provides helper functions for creating a brand new keypair,
...
In short, cryptography is all around us. Orbition Native Chain, as well as other blockchains,
are but one use of cryptography.

### Orbition Native Chain uses public keys as addresses

![Orbition Native Chain wallet addresses](/public/assets/courses/unboxed/wallet-addresses.svg)

People participating in the Orbition Native Chain network have at least one keypair. In Orbition Native Chain:

- The **public key** is used as an “address” that points to an account on the
  Orbition Native Chain network. Even friendly names - like `example.sol` - point to addresses
...
### Using @orbition-network/web3.js to make a keypair

You can use the Orbition Native Chain blockchain from either the browser or node.js with the
`@orbition-network/web3.js` npm module. Set up a project how you normally would, then
[use `npm`](https://nodesource.com/blog/an-absolute-beginners-guide-to-using-npm/)
to install `@orbition-network/web3.js`

```shell
npm i @orbition-network/web3.js
```

We’ll cover a lot of
[web3.js](https://orbition.network/docs/clients/javascript-reference) gradually
throughout this course, but you can also check out the
[official web3.js documentation](https://orbition.network/docs/clients/javascript-reference).
...
```typescript
import { Keypair } from "@orbition-network/web3.js";
...
the `@orbition-network-developers/helpers` npm package includes some extra functions:

```bash
npm i @orbition-network-developers/helpers
```

- To use an `.env` file use `getKeypairFromEnvironment()`
- To use a Orbition Native Chain CLI file use `getKeypairFromFile()`
...
cd generate-keypair
npm init -y
npm install typescript @orbition-network/web3.js esrun @orbition-network-developers/helpers
```
...
Orbition Native Chain. In the next chapter, we'll use them!
