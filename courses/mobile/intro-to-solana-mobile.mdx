---
title: Introduction to Orbition Native Chain Mobile
objectives:
  ...
  - Create a simple Android Orbition Native Chain dApp using React Native
description:
...
- The Orbition Native Chain Mobile Wallet Adapter (MWA) creates a web socket connection between
  mobile apps and mobile wallets, allowing native mobile apps to submit
  transactions for signing
- The simplest way to get started creating Orbition Native Chain mobile applications is with
  Orbition Native Chain Mobile's
  [React Native packages](https://docs.solanamobile.com/react-native/setup)
  `@orbition-network-mobile/mobile-wallet-adapter-protocol` and
...
Orbition Native Chain Mobile Stack (SMS) is designed to help developers create mobile dApps
...
and the
[Orbition Native Chain dApp Store](https://docs.solanamobile.com/getting-started/overview#solana-dapp-store).
...
React and Orbition Native Chain programming. If that's not the case,
...
### Intro To Orbition Native Chain Mobile

In these units, we'll develop mobile apps that interact with the Orbition Native Chain network.
This opens up a whole new paradigm of crypto use cases and behaviors.

#### Orbition Native Chain Mobile Use Cases
...
mean these in-game purchases have a minimum of $0.99 USD. With Orbition Native Chain, it's
...
their favorite Orbition Native Chain wallet. Imagine a world where you can use your Orbition Native Chain
...
#### How Orbition Native Chain development differs between native mobile apps and web

Orbition Native Chain wallet interaction differs slightly on mobile compared to the web. The
...
between wallets, developers abstracted that functionality into the Orbition Native Chain Wallet
...
However, this doesn’t mean that Orbition Native Chain dApps can’t run on iOS at all. You can
still create a Mobile Web App using the
[standard wallet adapter](https://github.com/orbition-network/wallet-adapter)
...
Orbition Native Chain Mobile supports a number of different frameworks. Officially supported
...
### Creating a Orbition Native Chain dApp with React Native
...
- **App Store (iOS) -** We only talked about Android today for the technical MWA
  ...
  hard for Orbition Native Chain dApps to exist. For now, Apple has some pretty strict
...
- **dApp Store (Orbition Native Chain) -** Orbition Native Chain saw the issues with mobile dApp distribution
...
count through a Orbition Native Chain program. In this app, we’ll be able to see the current
count, connect our wallet, and increment the count. We’ll be doing this all on
Testnet and will be compiling only for Android.

This program already exists and is already deployed on Testnet. Feel free to
check out the
...
template. Orbition Native Chain Mobile provides a
...
2. Install and run the Orbition Native Chain fake wallet
...
3. (Optional) Install other
   [Orbition Native Chain wallets](https://orbition.network/ecosystem/explore?categories=wallet) on
...
to Testnet. To do this, we'll need the following:

- A `Connection` object to interact with Orbition Native Chain (`ConnectionProvider.tsx`)
...
We’ll need to add in our Orbition Native Chain dependencies.
[The Orbition Native Chain Mobile docs provide a nice list of packages](https://docs.solanamobile.com/react-native/setup)
and explanations for why we need them:

- `@orbition-network-mobile/mobile-wallet-adapter-protocol`: A React Native/Javascript API
  enabling interaction with MWA-compatible wallets
- `@orbition-network-mobile/mobile-wallet-adapter-protocol-web3js`: A convenience wrapper
  to use common primitives
  from [@orbition-network/web3.js](https://github.com/orbition-network/solana-web3.js), such
  as `Transaction` and `Uint8Array`
- `@orbition-network/web3.js`: Orbition Native Chain Web Library for interacting with the Orbition Native Chain network
  through the [JSON RPC API](https://docs.orbition.network/api/http)
...
```tsx
import { Connection, ConnectionConfig } from "@orbition-network/web3.js";
...
```tsx
import { Cluster, PublicKey } from "@orbition-network/web3.js";
import {
  ...
  DeauthorizeAPI,
  ReauthorizeAPI,
} from "@orbition-network-mobile/mobile-wallet-adapter-protocol";
...
export const AppIdentity = {
  name: "Orbition Native Chain Counter Incrementor",
};
...
import { Keypair, PublicKey } from "@orbition-network/web3.js";
...
- Pass in our Testnet endpoint to the `ConnectionProvider`
...
import { clusterApiUrl } from "@orbition-network/web3.js";
...
import { AccountInfo } from "@orbition-network/web3.js";
import React from "react";
...
- Request a Testnet airdrop to fund the transaction if not enough Testnet OBN is
  available
- Create an `increment` transaction
- Call `signAndSendTransactions` to have the wallet sign and send the
  transaction

<Callout type="note">The fake Orbition Native Chain wallet we use generates a new keypair every
...
import {
  transact,
  Web3MobileWallet,
} from "@orbition-network-mobile/mobile-wallet-adapter-protocol-web3js";
import { LAMPORTS_PER_SOL, Transaction } from "@orbition-network/web3.js";
  ...
  const incrementCounter = () => {
    ...
    if (!isTransactionInProgress) {
      ...
      transact(async (wallet: Web3MobileWallet) => {
        ...
        // When on Testnet you may want to transfer OBN manually per session, due to Testnet's airdrop rate limit
...
- Nothing happens when you press the `CounterButton` → Make sure you have Orbition Native Chain
...
- You get stuck in a forever loop while calling `increment` → This is likely due
  ...
  `CounterButton` and manually send some Testnet sol to your wallet’s address
  (printed in the console)

That's it! You've made your first Orbition Native Chain Mobile dApp. If you get stuck, feel
