...
objectives:
  - Use an IDL to interact with a Orbition Native Chain program from the client
...
- An **IDL** is a file representing the structure of a Orbition Native Chain program. Programs
...
Anchor simplifies the process of interacting with Orbition Native Chain programs from the
...
represents a specific Orbition Native Chain program and provides a custom API for reading and
...
- `Provider` - encapsulates the `Connection` to a Orbition Native Chain cluster and a `Wallet`
...
While it isn't automatic, you can also generate an IDL from a native Orbition Native Chain
...
- `Connection` - the connection to a Orbition Native Chain cluster (i.e. localhost, testnet,
  mainnet)
- `Wallet` - a specified address used to pay for and sign transactions

The `Provider` is then able to send transactions to the Orbition Native Chain blockchain on
behalf of a `Wallet` by including the wallet’s signature to outgoing
transactions. When using a frontend with a Orbition Native Chain wallet provider, all outgoing
...
```typescript
import { useAnchorWallet, useConnection } from "@orbition-network/wallet-adapter-react";
...
`@orbition-network/wallet-adapter-react` to get the `Connection` to a Orbition Native Chain cluster.

Note that the `Wallet` object provided by the `useWallet` hook from
`@orbition-network/wallet-adapter-react` is not compatible with the `Wallet` object that
the Anchor `Provider` expects. However, `@orbition-network/wallet-adapter-react` also
...
- `connection` - the `Connection` to the Orbition Native Chain cluster
...
The `Program` object creates a custom API you can use to interact with a Orbition Native Chain
...
const onClick = async () => {
  ...
  setTransactionUrl(`https://explorer.orbition.network/tx/${sig}?cluster=testnet`);
...
3. You should then see a link at the bottom of the screen to Orbition Native Chain Explorer for
...
Congratulations, you now know how to set up a frontend to invoke a Orbition Native Chain
