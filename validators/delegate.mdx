...
- `node_pubkey` - The Orbition Network node that votes in this account.
...
- `account[0]` - RW - The VoteState. `VoteState::lockouts` and
  `VoteState::credits` are updated according to voting lockout rules see Tower BFT.
...
correctly. Voting incorrectly exposes that validator's stakes to slashing.

### Basics

The network pays rewards from a portion of network inflation. The number of
...
participation. The weighting unit is called a point.
...
sysvar that maps epochs to point values.
